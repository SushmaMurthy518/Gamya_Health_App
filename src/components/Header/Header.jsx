      ...
      <Navbar.Brand>
        ...
        <Navbar.Collapse id='basic-navbar-nav'>
          <Nav className='me-auto justify-content-end w-100'>
            ...
            <NavDropdown title="Our Courses" className='text-uppercase' id="basic-nav-dropdown">
              {courseDetails.map(category => (
                <NavDropdown key={category.category} title={category.category} id={`${category.category}-dropdown`}>
                  {category.courses.map(course => (
                    <NavDropdown.Item key={course.id} href={`/courses/${course.id}`}>
                      {course.title}
                    </NavDropdown.Item>
                  ))}
                </NavDropdown>
